...
| 1306, 1260, 1236, 1228, 1203, 1202, 1195, 1153 | `py/import-and-import-from` | tests and `voiage/cli.py` | Four current test modules now use explicit from-package module imports, removing redundant mixed import forms without changing imported objects. Other locations need current-path readback; no hosted alert dismissal is claimed. |
...
The repair changes only imports in tests/test_scientific_review_evidence.py, tests/test_belief_state_information.py, tests/test_implementation_information.py and tests/test_forecast_signal_information.py. Targeted tests passed. Required Protocol ellipses in voiage/metamodels.py and imported upstream Spack catalogue bytes are preserved. A fresh hosted scan must establish any resulting alert closure. No security gate is bypassed.
