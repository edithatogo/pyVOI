...
- [x] T1.2 — Freeze the minimal versioned event fields, severity policy and run/analysis/trace correlation mapping. (AC1).
- [x] T1.3 — Validate: Scientific results, audit evidence and operational logs have distinct contracts; diagnostics remain available with logging disabled. Record source, environment, command, exit status, test count and artifact digest; review diff and stop on unresolved failure (AC1).
...
- [x] T2.2 — Qualify tracing with minimal features and the supported MSRV; record compile-time and disabled-path costs. (AC2).
- [x] T2.3 — Validate: No exporter, network client or mandatory async runtime enters the numerical core. Record source, environment, command, exit status, test count and artifact digest; review diff and stop on unresolved failure (AC2).
...
- [x] T3.2 — Test host-installed subscriber coexistence before implementing application-owned configuration. (AC3).
- [x] T3.3 — Validate: Importing/calling the library never installs a global subscriber or replaces host handlers; repeated setup does not duplicate events. Record source, environment, command, exit status, test count and artifact digest; review diff and stop on unresolved failure (AC3).
...
- [x] T4.2 — Implement and test one Python-to-Rust correlation path and context isolation across parallel calls. (AC4).
- [x] T4.3 — Validate: Two concurrent runs do not exchange IDs; nested spans preserve parentage; absent tracing remains valid. Record source, environment, command, exit status, test count and artifact digest; review diff and stop on unresolved failure (AC4).
